...
namespace :report do
  ...
  task :export_to_zenodo => :environment do
    ...
    description = <<-EOS.gsub(/^\s*/, '').gsub("\s*\n\s*", " ")
      Article-Level Metrics (ALM) measure the reach and online engagement of scholarly articles.
      The #{sitenamelong} report contains the cumulative stats collected through
      #{publication_date.to_s(:long)} and is inclusive of all journals.
