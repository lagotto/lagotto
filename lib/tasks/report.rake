...
namespace :report do
  ...
  task :combined_stats => :environment do
    report = AlmCombinedStatsReport.new(
      alm_report:      AlmStatsReport.new(Source.active.public_sources),
      pmc_report:      PmcReport.new(Source.active.where(name: "pmc").first),
      counter_report:  CounterReport.new(Source.active.where(name:"counter").first),
      mendeley_report: MendeleyReport.new(Source.active.where(name:"mendeley").first)
    )
  ...
  task :combined_private_stats => :environment do
    report = AlmCombinedStatsReport.new(
      alm_report:      AlmStatsReport.new(Source.active),
      pmc_report:      PmcReport.new(Source.active.where(name: "pmc").first),
      counter_report:  CounterReport.new(Source.active.where(name:"counter").first),
      mendeley_report: MendeleyReport.new(Source.active.where(name:"mendeley").first)
    )
