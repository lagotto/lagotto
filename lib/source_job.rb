...
class SourceJob < Struct.new(:rs_ids, :source_id)
  ...
  def perform_get_data(rs)
    ...
    # We don't update retrieval status so that the request is repeated later.
    # We could get stuck, but we see this in alerts
    #
    # This mnethod returns a hash in the format { event_count: 12, previous_count: 8, skipped: true, update_interval: 31 }
    ...
    if data_from_source.is_a?(Hash)
      ...
      # ERROR
      return { event_count: nil, previous_count: previous_count, skipped: true, update_interval: update_interval }
    end
    ...
    if event_count.nil?
      rs.update_attributes(:retrieved_at => retrieved_at,
                           ...
                           :event_count => 0)
      { event_count: 0, previous_count: previous_count, skipped: true, update_interval: update_interval }
    else
      # SUCCESS
      if event_count > 0
        ...
        current_rev = save_alm_data("#{rs.source.name}:#{rs.article.doi_escaped}", data: data.clone, source_id: rs.source_id)

        data.except!(:history, :_attachments)
        data[:doc_type] = "history"
        history_rev = save_alm_data("#{rs.source.name}:#{rs.article.doi_escaped}:#{retrieved_at}", data: data, source_id: rs.source_id)
      ...
      { event_count: event_count, previous_count: previous_count, update_interval: update_interval }
    end
  end
