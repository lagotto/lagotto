  ...
  context "use background jobs" do
    ...
    context "queue all works" do
      it "queue" do
        ...
        expect(source.queue_all_works).to eq(10)
      end
      ...
      it "with inactive source" do
        ...
        expect(source.queue_all_works).to eq(0)
      end
      ...
      it "outside time interval" do
        ...
        allow(Delayed::Job).to receive(:enqueue).with(SourceJob.new(rs_ids, source.id), queue: source.name, run_at: Time.zone.now, priority: 5)
        expect(source.queue_all_works(start_date: Date.today - 2.days, end_date: Date.today - 2.days)).to eq(0)
      end
    end
    ...
    context "queue work jobs" do
      it "multiple works" do
        allow(Delayed::Job).to receive(:enqueue).with(SourceJob.new(rs_ids, source.id), queue: source.name, run_at: Time.zone.now, priority: 5)
        expect(source.queue_work_jobs(rs_ids)).to eq(10)
      end
      ...
      it "single work" do
        ...
        expect(source.queue_work_jobs([retrieval_status.id])).to eq(1)
      end
    end
