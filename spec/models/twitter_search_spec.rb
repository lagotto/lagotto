...
describe TwitterSearch, :type => :model do
  ...
  context "get_data" do
    ...
    it "should catch errors with the Twitter Search API" do
      ...
      expect(response).to eq(error: "the server responded with status 408 for https://api.twitter.com/1.1/search/tweets.json?q=%2210.1371%252Fjournal.pone.0000001%22%20OR%20%22http://www.plosone.org/work/info%253Adoi%252F10.1371%252Fjournal.pmed.0000001%22&count=100&include_entities=1&result_type=recent", :status=>408)
      expect(stub).to have_been_requested
  ...
  context "parse_data" do
    it "should report if there are no events and event_count returned by the Twitter Search API" do
      ...
      expect(subject.parse_data(result, work)).to eq(events: [], :events_by_day=>[], :events_by_month=>[], event_count: 0, events_url: "https://twitter.com/search?q=%2210.1371%252Fjournal.pone.0000000%22%20OR%20%22http://www.plosone.org/work/info%253Adoi%252F10.1371%252Fjournal.pmed.0000000%22&f=realtime", event_metrics: { pdf: nil, html: nil, shares: nil, groups: nil, comments: 0, likes: nil, citations: nil, total: 0 })
    end
