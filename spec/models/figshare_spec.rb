...
describe Figshare do
  ...
  context "use the figshare API" do
    it "should report if there are no events and event_count returned by the figshare API" do
      ...
      stub = stub_request(:get, subject.get_query_url(article)).to_return(:headers => { "Content-Type" => "application/json" }, :body => File.read(fixture_path + 'figshare_nil.json'), :status => 200)
      subject.get_data(article).should eq(events: [], event_count: nil)
    ...
    it "should report if there are events and event_count returned by the figshare API" do
      article = FactoryGirl.build(:article, :doi => "10.1371/journal.pone.0067729")
      stub = stub_request(:get, subject.get_query_url(article)).to_return(:headers => { "Content-Type" => "application/json" }, :body => File.read(fixture_path + 'figshare.json'), :status => 200)
