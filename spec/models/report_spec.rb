...
describe Report do
  ...
  context "error report" do
    ...
    it "generates proper links to the admin dashboard" do
      ...
      body_html.should include("<a href=\"http://#{CONFIG[:hostname]}/alerts\">Go to admin dashboard</a>")
    end
  end
  ...
  context "stale source report" do
    ...
    it "generates proper links to the admin dashboard" do
      ...
      mail = ActionMailer::Base.deliveries.last
      body_html = mail.body.parts.find { |p| p.content_type.match /html/ }.body.raw_source
      body_html.should include("<a href=\"http://#{CONFIG[:hostname]}/alerts?class=SourceNotUpdatedError\">Go to admin dashboard</a>")
    end
  end
end
