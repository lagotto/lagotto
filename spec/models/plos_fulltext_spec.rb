...
describe PlosFulltext, type: :model, vcr: true do
  ...
  context "parse_data" do
     it "should report that there are no events if the doi has the wrong prefix" do
      ...
      result = {}
      expect(subject.parse_data(result, work)).to eq(works: [], events: [{ source_id: "plos_fulltext", work_id: work.pid, total: 0, events_url: nil, extra: [], days: [], months: [] }])
    end
    ...
    it "should report if there are events returned by the PLOS Search API" do
      ...
      expect(related_work['URL']).to eq("http://dx.doi.org/10.1371/journal.pcbi.1003833")
