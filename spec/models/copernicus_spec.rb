...
describe Copernicus, :type => :model do
  ...
  context "get_data" do
    ...
    it "should catch authentication errors with the Copernicus API" do
      ...
      expect(response).to eq(error: "the server responded with status 401 for http://harvester.copernicus.org/api/v1/articleStatisticsDoi/doi:#{article.doi}", status: 401)
      expect(stub).to have_been_requested
    ...
    it "should catch timeout errors with the Copernicus API" do
      ...
      response = subject.get_data(article, options = { :source_id => subject.id })
      expect(response).to eq(error: "the server responded with status 408 for http://harvester.copernicus.org/api/v1/articleStatisticsDoi/doi:#{article.doi}", :status=>408)
      expect(stub).to have_been_requested
      expect(Alert.count).to eq(1)
      alert = Alert.first
