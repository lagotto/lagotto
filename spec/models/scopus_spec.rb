  ...
  context "use the Scopus API" do
    context "use article without events" do
      it "should report if there are no events and event_count returned by the Scopus API" do
        ...
        stub = stub_request(:get, scopus.get_query_url(article)).with(:headers => { 'Accept'=>'application/json', 'User-Agent'=>'Article-Level Metrics - http://example.com', 'X-ELS-APIKEY' => scopus.api_key, 'X-ELS-INSTTOKEN' => scopus.insttoken }).to_return(:headers => { "Content-Type" => "application/json" }, :body => File.read(fixture_path + 'scopus_nil.json'), :status => 200)
    ...
    context "use article with events" do
      ...
      it "should report if there are events and event_count returned by the Scopus API" do
        ...
        stub = stub_request(:get, scopus.get_query_url(article)).with(:headers => { 'Accept'=>'application/json', 'User-Agent'=>'Article-Level Metrics - http://example.com', 'X-ELS-APIKEY' => scopus.api_key, 'X-ELS-INSTTOKEN' => scopus.insttoken }).to_return(:headers => { "Content-Type" => "application/json" }, :body => body, :status => 200)
