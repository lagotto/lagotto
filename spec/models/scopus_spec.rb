...
describe Scopus do
  ...
  context "get_data" do
    ...
    it "should report if there are no events and event_count returned by the Scopus API" do
      ...
      stub = stub_request(:get, subject.get_query_url(article)).with(:headers => { 'Accept'=>'application/json', 'User-Agent'=>"Article-Level Metrics #{Rails.application.config.version} - http://#{CONFIG[:public_server]}", 'X-ELS-APIKEY' => subject.api_key, 'X-ELS-INSTTOKEN' => subject.insttoken }).to_return(:headers => { "Content-Type" => "application/json" }, :body => body, :status => 200)
