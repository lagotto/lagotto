...
describe Scopus, :type => :model do
  ...
  context "get_data" do
    ...
    it "should catch timeout errors with the Scopus API" do
      ...
      expect(response).to eq(error: "the server responded with status 408 for https://api.elsevier.com/content/search/index:SCOPUS?query=DOI(#{article.doi_escaped})", :status=>408)
      expect(stub).to have_been_requested
