...
describe PubMed do
  ...
  it "should report that there are no events if the doi and pmid are missing" do
    article_without_doi = FactoryGirl.build(:article, :doi => "", :pmid => "")
  ...
  context "use the PubMed API" do
    context "use article without events" do
      it "should not retrieve the PMCID for articles less than one month old" do
        article_recently_published = FactoryGirl.build(:article, :doi => "10.1371/journal.pone.0008776", :pmid => "17183631", :pmcid => "", :published_on => (Date.today - 7).to_s(:db))
      ...
      it "should report if there are no events and event_count returned by the PubMed API" do
        article_without_events = FactoryGirl.build(:article, :doi => "10.1371/journal.pone.0008776", :pmid => "1897483599")
    ...
    context "use article with events" do
      let(:article) { FactoryGirl.build(:article, :doi => "10.1371/journal.pone.0000001", :pmid => "17183631", :pmcid => "1762328") }
