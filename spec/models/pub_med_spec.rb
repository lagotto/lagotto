...
describe PubMed do
  ...
  context "use the PubMed API" do
    it "should report if there are no events and event_count returned by the PubMed API" do
      ...
      pub_med.get_data(article).should eq(events: [], event_count: 0, events_url: "http://www.ncbi.nlm.nih.gov/sites/entrez?db=pubmed&cmd=link&LinkName=pubmed_pmc_refs&from_uid=1897483599", event_metrics: { pdf: nil, html: nil, shares: nil, groups: nil, comments: nil, likes: nil, citations: 0, total: 0}, attachment: nil)
