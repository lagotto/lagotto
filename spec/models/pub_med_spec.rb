  ...
  context "use the PubMed API" do
    context "use article without events" do
      it "should not retrieve the PMCID for articles less than one month old" do
        ...
        stub_pmid_lookup = stub_request(:get, PubMed::EUTILS_URL + "term=#{article.doi}&field=DOI&db=pubmed&tool=#{PubMed::ToolID}").to_return(:body => File.read(fixture_path + 'pub_med_esearch_pmid_nil.xml'), :status => 200)
        stub_pmcid_lookup = stub_request(:get, PubMed::EUTILS_URL + "term=#{article.doi}&field=DOI&db=pmc&tool=#{PubMed::ToolID}").to_return(:body => File.read(fixture_path + 'pub_med_esearch_pmcid_nil.xml'), :status => 200)
