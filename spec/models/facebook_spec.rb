...
describe Facebook, type: :model, vcr: true do
  ...
  context "parse_data" do
    ...
    it "should report if the doi and canonical URL are missing" do
      ...
      expect(subject.parse_data(result, work)).to eq(metrics: { source: "facebook", work: work.pid, readers: 0, comments: 0, likes: 0, total: 0, extra: {} })
    end
    ...
    it "should report if there are events returned by the Facebook API" do
      ...
      result.extend Hashie::Extensions::DeepFetch
      response = subject.parse_data(result, work)
      expect(response[:metrics][:total]).to eq(9972)
      extra = response[:metrics][:extra]
  ...
  context "parse_data with url_linkstat" do
    ...
    it "should report if there are no events returned by the Facebook API" do
      ...
      result.extend Hashie::Extensions::DeepFetch
      response = subject.parse_data(result, work)
      expect(response[:metrics][:extra]).to eq([{"url"=>"http://dx.doi.org/10.1371/journal.pone.0000001", "share_count"=>0, "like_count"=>0, "comment_count"=>0, "click_count"=>0, "total_count"=>0, "comments_fbid"=>nil}, {"url"=>"http://www.plosmedicine.org/article/info:doi/10.1371/journal.pone.0000001", "share_count"=>0, "like_count"=>0, "comment_count"=>0, "click_count"=>0, "total_count"=>0, "comments_fbid"=>"10150168740355926"}])
      expect(response[:metrics][:total]).to eq(0)
      expect(response[:metrics][:readers]).to eq(0)
      expect(response[:metrics][:comments]).to eq(0)
