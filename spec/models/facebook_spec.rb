...
describe Facebook, :type => :model do
  ...
  context "get_data" do
    ...
    it "should catch authorization errors with the Facebook API" do
      ...
      expect(response).to eq(error: "the server responded with status 401 for #{subject.get_query_url(article)}", status: 401)
      expect(stub).to have_been_requested
  ...
  context "get_data with linkstat_url" do
    ...
    it "should catch timeout errors with the Facebook API" do
      ...
      stub = stub_request(:get, subject.get_query_url(article)).to_return(:status => [408])
      response = subject.get_data(article, options = { :source_id => subject.id })
      expect(response).to eq(error: "the server responded with status 408 for #{subject.get_query_url(article)}", :status=>408)
      expect(stub).to have_been_requested
      expect(Alert.count).to eq(1)
      alert = Alert.first
  ...
  context "parse_data" do
    ...
    it "should report if the doi and canonical URL are missing" do
      ...
      expect(subject.parse_data(result, article)).to eq(:events=>{}, :events_by_day=>[], :events_by_month=>[], :events_url=>nil, :event_count=>0, :event_metrics=>{:pdf=>nil, :html=>nil, :shares=>0, :groups=>nil, :comments=>0, :likes=>0, :citations=>nil, :total=>0})
    end
