...
describe Wos do
  ...
  context "use the Wos API" do
    ...
    it "should report if there are no events and event_count returned by the Wos API" do
      ...
      subject.parse_data(article).should eq(:events => 0, :event_count => 0, :events_url => nil, event_metrics: { pdf: nil, html: nil, shares: nil, groups: nil, comments: nil, likes: nil, citations: 0, total: 0 }, :attachment => nil)
    ...
    it "should catch errors with the Wos API" do
      ...
      subject.parse_data(article, options = { :source_id => subject.id }).should eq(events: [], event_count: nil)
      stub.should have_been_requested
      Alert.count.should == 1
      alert = Alert.first
