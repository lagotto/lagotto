...
describe Nature do
  ...
  context "use the Nature Blogs API" do
    it "should report if there are no events and event_count returned by the Nature Blogs API" do
      ...
      stub = stub_request(:get, nature.get_query_url(article_without_events)).to_return(:headers => { "Content-Type" => "application/json" }, :body => File.read(fixture_path + 'nature_nil.json'), :status => 200)
    ...
    it "should report if there are events and event_count returned by the Nature Blogs API" do
      ...
      stub = stub_request(:get, nature.get_query_url(article)).to_return(:headers => { "Content-Type" => "application/json" }, :body => File.read(fixture_path + 'nature.json'), :status => 200)
