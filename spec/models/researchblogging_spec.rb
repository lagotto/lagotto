...
describe Researchblogging do
  ...
  it "should report that there are no events if the doi is missing" do
    ...
    researchblogging.parse_data(article_without_doi).should eq(events: [], event_count: nil)
  end

  context "use the ResearchBlogging API" do
    let(:auth) { ActionController::HttpAuthentication::Basic.encode_credentials(researchblogging.username, researchblogging.password) }
    it "should report if there are no events and event_count returned by the ResearchBlogging API" do
      ...
      researchblogging.parse_data(article).should eq(events: [], event_count: 0, event_metrics: { pdf: nil, html: nil, shares: nil, groups: nil, comments: nil, likes: nil, citations: 0, total: 0 }, events_url: researchblogging.get_events_url(article))
    ...
    it "should report if there are events and event_count returned by the ResearchBlogging API" do
      ...
      response = researchblogging.parse_data(article)
    ...
    it "should catch errors with the ResearchBlogging API" do
      ...
      researchblogging.parse_data(article, options = { :source_id => researchblogging.id }).should be_nil
      stub.should have_been_requested
