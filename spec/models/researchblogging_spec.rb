...
describe Researchblogging do
  ...
  context "use the ResearchBlogging API" do
    ...
    it "should report if there are events and event_count returned by the ResearchBlogging API" do
      ...
      stub = stub_request(:get, "http://#{researchblogging.username}:#{researchblogging.password}@researchbloggingconnect.com/blogposts?article=doi:#{CGI.escape(article.doi)}&count=100").to_return(:body => body, :status => 200)
