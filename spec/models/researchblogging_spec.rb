...
describe Researchblogging, :type => :model do
  ...
  context "get_data" do
    ...
    it "should catch errors with the ResearchBlogging API" do
      ...
      expect(response).to eq(error: "the server responded with status 408 for http://researchbloggingconnect.com/blogposts?count=100&article=doi:#{article.doi_escaped}", :status=>408)
      expect(stub).to have_been_requested
  ...
  context "parse_data" do
    ...
    it "should report if there are no events and event_count returned by the ResearchBlogging API" do
      ...
      expect(response).to eq(events: [], :events_by_day=>[], :events_by_month=>[], event_count: 0, event_metrics: { pdf: nil, html: nil, shares: nil, groups: nil, comments: nil, likes: nil, citations: 0, total: 0 }, events_url: subject.get_events_url(article))
    end
