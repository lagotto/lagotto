...
describe PmcEuropeData, :type => :model do
  ...
  context "get_data" do
    ...
    it "should catch errors with the PMC Europe API" do
      ...
      expect(response).to eq(error: "the server responded with status 408 for http://www.ebi.ac.uk/europepmc/webservices/rest/MED/#{article.pmid}/databaseLinks//1/json", status: 408)
      expect(stub).to have_been_requested
  ...
  context "get_data via accession_id" do
    ...
    it "should catch errors with the PMC Europe API" do
      ...
      stub = stub_request(:get, subject.get_query_url(article)).to_return(:status => [408])
      response = subject.get_data(article, source_id: subject.id)
      expect(response).to eq(error: "the server responded with status 408 for http://www.ebi.ac.uk/europepmc/webservices/rest/search/query=ACCESSION_ID:#{article.doi}", status: 408)
      expect(stub).to have_been_requested
      expect(Alert.count).to eq(1)
      alert = Alert.first
      expect(alert.class_name).to eq("Net::HTTPRequestTimeOut")
  ...
  context "parse_data" do
    ...
    it "should report if there are no events and event_count returned by the PMC Europe API" do
      ...
      expect(response).to eq(events: [], :events_by_day=>[], :events_by_month=>[], event_count: 0, events_url: "http://europepmc.org/abstract/MED/#{article.pmid}#fragment-related-bioentities", event_metrics: { pdf: nil, html: nil, shares: nil, groups: nil, comments: nil, likes: nil, citations: 0, total: 0 })
    end
    ...
    it "should report if there are events and event_count returned by the PMC Europe API" do
      ...
      result = JSON.parse(body)
      response = subject.parse_data(result, article)
      expect(response).to eq(events: { "EMBL" => 10, "UNIPROT" => 21700 }, :events_by_day=>[], :events_by_month=>[], event_count: 21710, events_url: "http://europepmc.org/abstract/MED/#{article.pmid}#fragment-related-bioentities", event_metrics: { pdf: nil, html: nil, shares: nil, groups: nil, comments: nil, likes: nil, citations: 21710, total: 21710 })
    end
