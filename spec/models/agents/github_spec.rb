...
describe Github, type: :model, vcr: true do
  ...
  context "parse_data" do
    ...
    it "should report if the canonical_url is missing" do
      work = FactoryGirl.create(:work, :canonical_url => nil)
      result = {}
      expect(subject.parse_data(result, work_id: work.id)).to eq(:events=>[{:source_id=>"github", :work_id=>work.pid, :readers=>0, :total=>0, :extra=>{}}])
    end
    ...
    it "should report if there are no events and event_count returned by the Github API" do
      ...
      expect(response).to eq(events: [{ source_id: "github", work_id: work.pid, readers: 0, total: 0, extra: extra }])
    end
