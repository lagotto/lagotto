...
describe Openedition do
  ...
  context "use the Openedition API" do
    it "should report if there are no events and event_count returned by the Openedition API" do
      ...
      subject.parse_data(article).should eq(events: [], events_url: "http://search.openedition.org/index.php?op%5B%5D=AND&q%5B%5D=#{article.doi_escaped}&field%5B%5D=All&pf=Hypotheses.org", event_count: 0, event_metrics: { pdf: nil, html: nil, shares: nil, groups: nil, comments: nil, likes: nil, citations: 0, total: 0})
