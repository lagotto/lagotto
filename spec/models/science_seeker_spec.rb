...
describe ScienceSeeker, type: :model, vcr: true do
  ...
  context "parse_data" do
    ...
    it "should report if there are events and event_count returned by the ScienceSeeker API" do
      ...
      expect(event[:days].first).to eq(year: 2012, month: 5, day: 11, total: 1)
      ...
      expect(related_work['related_works']).to eq([{"related_work"=>"doi:10.1371/journal.pone.0035869", "source"=>"scienceseeker", "relation_type"=>"discusses"}])
    ...
    it "should report if there is one event returned by the ScienceSeeker API" do
      ...
      expect(event[:days].first).to eq(year: 2012, month: 5, day: 18, total: 1)
      expect(event[:months].length).to eq(1)
