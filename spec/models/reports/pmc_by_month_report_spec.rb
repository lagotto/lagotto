...
describe PmcByMonthReport do
  include Dateable

  subject(:report){ PmcByMonthReport.new(source, format:format, year:year, month:month) }
  ...
  it "must be constructed with a :format, :year, and :month" do
    expect {
      PmcByMonthReport.new
    }.to raise_error(ArgumentError, /missing keywords.*format.*year.*month/)

    expect {
      PmcByMonthReport.new(source, format:format, year:year, month:month)
    }.to_not raise_error
  end
  ...
  describe "#line_items" do
    describe "when there are no Pmc months or works" do
