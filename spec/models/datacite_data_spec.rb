...
describe DataciteData, type: :model, vcr: true do
  ...
  context "parse_data" do
    it "should report if the doi is missing" do
      work = FactoryGirl.create(:work, :doi => nil)
      result = {}
      expect(subject.parse_data(result, work)).to eq(works: [], metrics: { source: "datacite_data", work: work.pid, total: 0, days: [], months: [] })
    end
    ...
    it "should report if there are events and event_count returned by the Datacite API" do
      ...
      expect(response[:metrics][:events_url]).to eq("http://search.datacite.org/ui?q=doi:#{work.doi_escaped}")
