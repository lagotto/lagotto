...
describe Mendeley do
  ...
  context "use the Mendeley API" do
    it "should report if there are no events and event_count returned by the Mendeley API" do
      ...
      stub = stub_request(:get, @mendeley.search_url(CGI.escape(CGI.escape(article_without_events.doi)), "doi")).to_return(:body => File.read(fixture_path + 'mendeley_nil.json'), :status => 200)
    ...
    it "should report if there are events and event_count returned by the Mendeley API" do
      ...
      stub = stub_request(:get, @mendeley.search_url(CGI.escape(CGI.escape(@article.doi)), "doi")).to_return(:body => File.read(fixture_path + 'mendeley.json'), :status => 200)
      stub_related = stub_request(:get, @mendeley.related_url("46cb51a0-6d08-11df-afb8-0026b95d30b2")).to_return(:body => File.read(fixture_path + 'mendeley_related.json'), :status => 200)
    ...
    it "should catch errors with the Mendeley API" do
      ...
      stub = stub_request(:get, @mendeley.search_url(CGI.escape(CGI.escape(@article.doi)), "doi")).to_return(:status => 408)
