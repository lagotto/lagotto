...
describe Mendeley do
  ...
  context "use the Mendeley API for uuid lookup" do
    ...
    it "should return the Mendeley uuid by the Mendeley API" do
      stub = stub_request(:get, mendeley.get_query_url(article.pub_med, "pmid")).to_return(:headers => { "Content-Type" => "application/json" }, :body => File.read(fixture_path + 'mendeley.json'), :status => 200)
    ...
    it "should return the Mendeley uuid by searching the Mendeley API" do
      ...
      stub = stub_request(:get, mendeley.get_query_url(article.pub_med, "pmid")).to_return(:headers => { "Content-Type" => "application/json" }, :body => File.read(fixture_path + 'mendeley_nil.json'), :status => 200)
      stub_doi = stub_request(:get, mendeley.get_query_url(CGI.escape(article.doi_escaped), "doi")).to_return(:headers => { "Content-Type" => "application/json" }, :body => File.read(fixture_path + 'mendeley_nil.json'), :status => 200)
      stub_title = stub_request(:get, mendeley.get_query_url(CGI.escape(article.title_escaped), "title")).to_return(:headers => { "Content-Type" => "application/json" }, :body => File.read(fixture_path + 'mendeley_search.json'), :status => 200)
    ...
    it "should return nil for the Mendeley uuid if the Mendeley API returns incomplete response" do
      stub = stub_request(:get, mendeley.get_query_url(article.pub_med, "pmid")).to_return(:headers => { "Content-Type" => "application/json" }, :body => File.read(fixture_path + 'mendeley_incomplete.json'), :status => 200)
      stub_doi = stub_request(:get, mendeley.get_query_url(CGI.escape(article.doi_escaped), "doi")).to_return(:headers => { "Content-Type" => "application/json" }, :body => File.read(fixture_path + 'mendeley_incomplete.json'), :status => 200)
      stub_title = stub_request(:get, mendeley.get_query_url(CGI.escape(article.title_escaped), "title")).to_return(:headers => { "Content-Type" => "application/json" }, :body => File.read(fixture_path + 'mendeley_search.json'), :status => 200)
      mendeley.get_mendeley_uuid(article).should be_nil
      stub.should have_been_requested
      stub_doi.should have_been_requested
  ...
  context "use the Mendeley API for metrics" do
    it "should report if there are no events and event_count returned by the Mendeley API" do
      ...
      stub = stub_request(:get, mendeley.get_query_url(article.mendeley)).to_return(:headers => { "Content-Type" => "application/json" }, :body => File.read(fixture_path + 'mendeley_error.json'), :status => 404)
    ...
    it "should report if there are events and event_count returned by the Mendeley API" do
      ...
      stub = stub_request(:get, mendeley.get_query_url(article.mendeley)).to_return(:headers => { "Content-Type" => "application/json" }, :body => body, :status => 200)
    ...
    it "should report no events and event_count if the Mendeley API returns incomplete response" do
      ...
      stub = stub_request(:get, mendeley.get_query_url(article.mendeley)).to_return(:headers => { "Content-Type" => "application/json" }, :body => File.read(fixture_path + 'mendeley_incomplete.json'), :status => 200)
      mendeley.get_data(article).should eq({ :events => [], :event_count => 0 })
      stub.should have_been_requested
      Alert.count.should == 0
    end
    ...
    it "should report no events and event_count if the Mendeley API returns malformed response" do
      article = FactoryGirl.build(:article, :doi => "10.1371/journal.pone.0044294")
      stub = stub_request(:get, mendeley.get_query_url(article.mendeley)).to_return(:headers => { "Content-Type" => "application/json" }, :body => File.read(fixture_path + 'mendeley_nil.json'), :status => 404)
      mendeley.get_data(article).should eq({ :events => [], :event_count => 0 })
      stub.should have_been_requested
      Alert.count.should == 0
    end
