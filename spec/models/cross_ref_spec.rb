...
describe CrossRef do
  ...
  context "use the CrossRef API" do
    ...
    it "should report if there are no events and event_count returned by the CrossRef API" do
      ...
      subject.parse_data(article).should eq(events: [], event_count: 0, event_metrics: { pdf: nil, html: nil, shares: nil, groups: nil, comments: nil, likes: nil, citations: 0, total: 0 })
  ...
  context "use the CrossRef OpenURL API" do
    ...
    it "should report if there is an event_count of zero returned by the CrossRef OpenURL API" do
      stub = stub_request(:get, subject.get_query_url(article)).to_return(:body => File.read(fixture_path + 'cross_ref_openurl_nil.xml'), :status => 200)
      subject.parse_data(article).should eq(events: [], event_count: 0, event_metrics: { pdf: nil, html: nil, shares: nil, groups: nil, comments: nil, likes: nil, citations: 0, total: 0 })
      stub.should have_been_requested
    end

    it "should report if there is an event_count greater than zero returned by the CrossRef OpenURL API" do
      stub = stub_request(:get, subject.get_query_url(article)).to_return(:body => File.read(fixture_path + 'cross_ref_openurl.xml'), :status => 200)
      response = subject.parse_data(article)
