  ...
  context "use the PMC Europe API" do
    context "use article without events" do
      it "should report if there are no events and event_count returned by the PMC Europe API" do
        ...
        subject.parse_data(article).should eq(event_count: 0, events_url: "http://europepmc.org/abstract/MED/#{article.pmid}#fragment-related-citations", event_metrics: { pdf: nil, html: nil, shares: nil, groups: nil, comments: nil, likes: nil, citations: 0, total: 0 })
    ...
    context "use article with events" do
      ...
      it "should report if there are events and event_count returned by the PMC Europe API" do
        ...
        subject.parse_data(article).should eq(event_count: 23, events_url: "http://europepmc.org/abstract/MED/#{article.pmid}#fragment-related-citations", event_metrics: {pdf: nil, html: nil, shares: nil, groups: nil, comments: nil, likes: nil, citations: 23, total: 23 })
        stub.should have_been_requested
      end
