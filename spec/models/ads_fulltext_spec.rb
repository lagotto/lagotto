...
describe AdsFulltext, type: :model, vcr: true do
  ...
  context "get_data" do
    ...
    it "should catch errors with the ADS API" do
      ...
      expect(response).to eq(error: "the server responded with status 408 for http://adsws-staging.elasticbeanstalk.com/v1/search/query?q=%22body%3A#{work.doi_escaped}%22&start=0&rows=100&fl=author%2Ctitle%2Cpubdate%2Cidentifier%2Cdoi&access_token=EXAMPLE", status: 408)
      expect(stub).to have_been_requested
