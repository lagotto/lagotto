...
describe Citeulike do
  ...
  context "use the CiteULike API" do
    it "should report if there are no events and event_count returned by the CiteULike API" do
      ...
      stub = stub_request(:get, subject.get_query_url(article_without_events)).to_return(:body => body, :status => 404)
      subject.get_data(article_without_events).should eq(events: [], events_url: subject.get_events_url(article_without_events), event_count: 0, event_metrics: { pdf: nil, html: nil, shares: 0, groups: nil, comments: nil, likes: nil, citations: nil, total: 0 }, attachment: nil)
    ...
    it "should report if there are events and event_count returned by the CiteULike API" do
      ...
      response[:attachment][:data].should eq("<?xml version=\"1.0\"?>\n" + body)
