...
describe DataciteImport, type: :model, vcr: true do
  ...
  context "get_query_url" do
    ...
    it "with member_symbol" do
      FactoryGirl.create(:publisher, member_symbol: "CDL.DRYAD", service: "datacite")
      expect(subject.get_query_url).to eq("http://search.datacite.org/api?q=*%3A*&start=0&rows=1000&fl=doi%2Ccreator%2Ctitle%2Cpublisher%2CpublicationYear%2CresourceTypeGeneral%2Cdatacentre_symbol%2CrelatedIdentifier%2Cupdated&fq=updated%3A%5B2015-04-07T00%3A00%3A00Z+TO+2015-04-08T23%3A59%3A59Z%5D&fq=datacentre_symbol%3ACDL.DRYAD&fq=has_metadata%3Atrue&fq=is_active%3Atrue&wt=json")
    end
  ...
  context "get_total" do
    it "with works" do
      expect(subject.get_total).to eq(5634618)
    end

    it "with no works" do
      expect(subject.get_total(from_date: "2014-12-25", until_date: "2014-12-25")).to eq(0)
    end
  end
  ...
  context "get_data" do
    ...
    it "should report if there are works returned by the Datacite Metadata Search API" do
      ...
      expect(response["response"]["numFound"]).to eq(5634618)
      doc = response["response"]["docs"].first
      expect(doc["doi"]).to eq("10.5517/CCPW8WP")
    end
