...
describe SourceJob do
  ...
  it "should perform and get skipped" do
    ...
    stub = stub_request(:get, retrieval_status.source.get_query_url(CGI.escape(CGI.escape(retrieval_status.article.doi)), "doi")).to_return(:body => File.read(fixture_path + 'mendeley_nil.json'), :status => 200)
    ...
    stub_title = stub_request(:get, retrieval_status.source.get_query_url(CGI.escape(CGI.escape(retrieval_status.article.title)), "title")).to_return(:body => File.read(fixture_path + 'mendeley_nil.json'), :status => 200)
    result = source_job.perform_get_data(retrieval_status.id)
  ...
  it "should perform and get error" do
    ...
    result = source_job.perform_get_data(retrieval_status.id)
    rs = result[:retrieval_status]
    result[:retrieval_history].should be_nil
    rs[:event_count].should eq(retrieval_status.event_count)
    rs[:id].should eq(retrieval_status.id)
    rs[:scheduled_at].should == scheduled_at
