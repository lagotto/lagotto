  ...
  context "caching", :caching => true do
    ...
    context "index" do
      ...
      it "can cache articles" do
        expect(Rails.cache.exist?("rabl/v5/#{cache_key_list}//hash")).to be false
    ...
    context "article is updated" do
      ...
      it "does not use a stale cache when an article is updated" do
        expect(Rails.cache.exist?("#{key}//hash")).to be false
        get uri, nil, 'HTTP_ACCEPT' => 'application/json'
        expect(last_response.status).to eq(200)

        sleep 1

        expect(Rails.cache.exist?("#{key}//hash")).to be true
        ...
        expect(Rails.cache.exist?("#{cache_key}//hash")).to be true
