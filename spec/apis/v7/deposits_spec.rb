...
describe "/api/v7/deposits", :type => :api do
  ...
  # Different to `retrieved_deposit` in that it doesn't contain `prefix`, which is generated by PrefixProcessor in a DepositJob.
  # TODO JW inconsistencies in JSON encoding means that `false` is encoded with a string here but not in `retrieved_deposit`.
  ...
  # The Deposit as it should be retrieved. In this case it would have been created by a FactoryGirl create. 
  let(:retrieved_deposit) { { "id"=>deposit.uuid,
                  "state"=>"waiting",
                  "message_type"=>"relation",
                  "message_action"=>"create",
                  "source_token"=>"citeulike_123",
  ...
  context "show" do
    ...
    context "as admin user" do
      it "JSON" do
        ...
        expect(response["meta"]["error"]).to be_nil
        expect(response["deposit"]).to eq(retrieved_deposit)
      end
    end
