class CounterByMonthReport
  include Reportable::ToCsv
  include Dateable

  class Query
    attr_reader :counter, :starting_year, :starting_month

    def initialize(counter, starting_year:, starting_month:)
      @counter = counter
  ...
  def initialize(counter, format:, year:, month:)
    @format = format
