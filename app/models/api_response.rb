class ApiResponse < ActiveRecord::Base
  ...
  scope :increasing, lambda { |number, source_ids| where("update_interval IS NOT NULL AND (event_count - previous_count) / IF(update_interval = 0, 1, update_interval) >= ? AND source_id IN (?)", number, source_ids) }
