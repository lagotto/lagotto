  ...
  def by_month
    ...
    when "plos_comments"
      if events.blank?
        ...
        events.group_by {|event| event["event"]["created"].to_datetime.strftime("%Y-%m") }.sort.map {|k,v| { :year => k[0..3].to_i, :month => k[5..6].to_i, :pdf => nil, :html => nil, :shares => nil, :groups => nil, :comments => v.length, :likes => nil, :citations => nil, :total => v.length }}
      end
    when "researchblogging"
      if events.blank?
        nil
