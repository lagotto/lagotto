class SourceDecorator < Draper::Decorator
  ...
  def status
    { "refreshed" => work_count - (stale_count + queued_count),
      "queued" => queued_count,
      "stale" => stale_count }
  end
  ...
  def by_day
    { "with_events" => with_events_by_day_count,
      ...
      "not_updated" => work_count - (with_events_by_day_count + without_events_by_day_count) }
  end

  def by_month
    { "with_events" => with_events_by_month_count,
      "without_events" => without_events_by_month_count,
      "not_updated" => work_count - (with_events_by_month_count + without_events_by_month_count) }
  end
end
