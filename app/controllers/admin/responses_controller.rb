class Admin::ResponsesController < Admin::ApplicationController
  ...
  def index
    if request.xhr?
      ...
      errors = ErrorMessage.unscoped.includes(:source).where("sources.active = 1 AND error_messages.created_at > NOW() - INTERVAL 24 HOUR AND sources.active = 1").order("group_id, display_name").group(:source_id).count
