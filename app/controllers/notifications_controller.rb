  ...
  def load_index
    ...
    if params[:source_id]
      collection = collection.includes(:source)
                   ...
                   .references(:source)
      @source_group = collection.group(:source_id).count.map { |s| [cached_source_names.fetch(s[0], {}).fetch(:name, nil), s[1]] }.first
    ...
    @sources = collection.where.not("notifications.source_id IS NULL").group("notifications.source_id").count.map { |s| [cached_source_names.fetch(s[0], {}).fetch(:name, nil), s[1]] }
