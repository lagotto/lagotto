class DeleteEventJob < ActiveJob::Base
  ...
  def perform(source)
    ...
    work_ids = ReferenceRelation.where(source_id: source.id).select(:work_id, :related_work_id).group(:work_id, :related_work_id).having("count(*) = 1").pluck(:work_id)
    Work.where(id: work_ids).destroy_all
    source.reference_relations.destroy_all
