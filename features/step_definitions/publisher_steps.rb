...
Then /^I should see the publisher "(.*?)"$/ do |name|
  ...
  page.has_css?('a', :text => name).should be true
end

Then /^I should see the alert "(.*?)"$/ do |text|
  # page.driver.render("tmp/capybara/alert.png")
  page.has_css?('div.alert', :text => text, :visible => true).should be true
end

Then /^the CrossRef ID should be (\d+)$/ do |id|
  within(".dl-horizontal") do
    page.has_css?('a', :text => id, :visible => true).should be true
  end
end

Then /^the DOI prefix should be "(.*?)"$/ do |prefix|
  within(".dl-horizontal") do
    page.has_css?('dd', :text => prefix, :visible => true).should be true
  end
end

Then /^the other names should include "(.*?)"$/ do |name|
  within(".dl-horizontal") do
    page.has_css?('dd', :text => name, :visible => true).should be true
  end
end
