class RenamePmcEurope < ActiveRecord::Migration
  ...
  def down
    europe_pmc = Source.where(type: "EuropePmc").update_all(type: "PmcEurope") if europe_pmc
    europe_pmc_data = Source.where(type: "EuropePmcData").update_all(type: "PmcEuropeData") if europe_pmc_data
  end
end
